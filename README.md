...
a rich text editor produces a file that can handle fancy colors, table, bullets, and highlighting etc. while a text editor cannot! it can only read plain text files with nothing else just purely undecorated texts!! (so that's why when you try to open a .docx file in notepad you get gibberish letters, because it can't parse those to .txt format) 
